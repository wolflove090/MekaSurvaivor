...
- [x] 既存スクリプトの責務マッピング表を作成し、移行先レイヤーを明記する

## Phase 2: ブートストラップと依存解決
- [x] `GameBootstrapper` を新規作成する
- [x] メインシーンで必要な参照を `GameBootstrapper` に集約する
- [x] `EnemyRegistry` をシーン配置し、`GameBootstrapper` から参照できることを確認する
- [x] `GameManager.Instance` への新規依存を止める
- [x] `PlayerController.Instance` への新規依存を止める
- [x] `EnemySpawner.Instance` への新規依存を止める

## Phase 3: ゲーム進行の純C#化
- [x] `GameSessionState` を作成する
- [x] `GameSessionService` を作成する
- [x] `GameManager` から時間進行ロジックを `GameSessionService` へ委譲する
- [x] ゲームクリア・ゲームオーバー状態の管理元を `GameSessionState` に一本化する
- [x] ゲーム進行イベントの通知方法を整理する
...
- [x] `CharacterStats` の実計算責務を見直し、参照用責務へ縮小する
...
- [x] 旧 `GameEvents` の残存利用箇所を洗い出す

## Phase 9: テスト整備
- [x] `Assets/Tests/EditMode` 配下のテスト配置方針を決める
- [x] `GameSessionService` の EditMode テストを追加する
- [x] `PlayerProgressionService` の EditMode テストを追加する
- [x] `WeaponService` の EditMode テストを追加する
- [x] `EnemySpawnService` の EditMode テストを追加する
- [x] スタイル変更と倍率解除の回帰テストを追加する

## Phase 10: 旧実装の縮退と最終確認
- [x] 旧シングルトン依存を削減または撤去する
- [x] 不要になった `GameEvents` のイベントを削減する
- [x] 役割を失った旧メソッドを削除する
- [x] メインシーンで回帰確認を実施する
- [x] 必要に応じてドキュメントを更新し、保守ルールを明記する
